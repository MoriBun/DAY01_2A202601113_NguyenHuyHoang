...
def call_openai(
    ...
    response = client.chat.completions.create(
        ...
        temperature=temperature,   # 0.0 = ổn định, càng cao càng "sáng tạo"
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        lines.append(
            f"{_trim(r['prompt']):<40} | "
            f"{_trim(r['gpt4o_response']):<40} | "
            f"{_trim(r['mini_response']):<40} | "
            f"{r['gpt4o_latency']:>13.2f}s | "
            f"{r['mini_latency']:>11.2f}s"
        )
    return "\n".join(lines)
